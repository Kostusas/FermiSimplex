...
RitzMinEigenvalueBounds tridiagonal_min_eigenvalue_bounds(
    ...
    if (size > static_cast<size_t>(std::numeric_limits<int>::max())) {
        throw std::runtime_error("Lanczos: tridiagonal dimension exceeds LP64 range");
    }
    ...
    const auto theta_min = eigenvalues.front();
    ...
    const auto residual =
        ...
        std::abs(eigenvector_at(size - 1, 0, eigenvectors));
...
double min_eigenvalue_lanczos(
    ...
    for (size_t iteration = 0; iteration < size; ++iteration) {
        std::copy(
            q.begin(),
            q.end(),
            basis.begin() + static_cast<std::ptrdiff_t>(iteration * size)
        );
        matvec(q, z);
        if (iteration > 0) {
            axpy(
        ...
        for (size_t basis_index = 0; basis_index <= iteration; ++basis_index) {
            ...
            axpy(-overlap, basis_vector, std::span<Complex>(z.data(), z.size()));
        }
