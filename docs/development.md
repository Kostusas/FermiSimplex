...
LinearTetrahedron keeps the reusable numerical library and Python interface in
separate source trees:

```text
cpp/include/lineartetrahedron/   public C++ API
...
python/lineartetrahedron/        small public Python layer
...
moments, and adaptive-loop mechanics. LinearTetrahedron adds only the spectral
...
cmake -S cpp -B build/core \
  -DLINEARTETRAHEDRON_BUILD_TESTING=ON
...
```cmake
find_package(lineartetrahedron CONFIG REQUIRED)
target_link_libraries(my_target PRIVATE lineartetrahedron::core)
```
...
- `LINEARTETRAHEDRON_BUILD_PYTHON`
